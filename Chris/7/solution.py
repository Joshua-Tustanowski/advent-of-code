...
def read_and_parse_input_file_into_graph(file_name: str) -> dict:
    ...
    with open(file_name, 'r') as file:
        ...
        return {holding_bag_regex.findall(line)[0]:
...
def num_possible_containers(graph, target_colour):
    ...
    def has_target_colour(colour: str, target_colour: str) -> bool:
        ...
        return any(inner_colour == target_colour or has_target_colour(inner_colour, target_colour)
                   for num_colour, inner_colour in colour_contents)

    return sum(has_target_colour(outermost_colour, target_colour) for outermost_colour in graph
...
def total_inner_bags(graph, start_colour):
    ...
    def num_bags_contained(colour: str) -> 0:
        ...
        return sum(num_inner_bags * (1+num_bags_contained(inner_colour))
                   for num_inner_bags, inner_colour in bag_contents)
